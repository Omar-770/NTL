...
{
    if (!ntl_profiles(wpd.get_ntl2(), folder, file + "_ntl2"))   return false;
    if (!ntl_profiles(wpd.get_ntl3(), folder, file + "_ntl3"))   return false;
    ...
    const std::string& folder, const std::string& file)
{
    ...
    std::ofstream sp_file(dir / (file + "_code_sparams.csv"));
    std::ofstream f_targets(dir / (file + "_code_targets.csv"));

    if (!sp_file.is_open()) return false;

    // Header: Freq in GHz, S-parameters in dB
    sp_file << "\"Freq [GHz]\",\"dB(St(1,1)) []\",\"dB(St(1,2)) []\",\"dB(St(2,2)) []\",\"dB(St(1,3)) []\",\"dB(St(2,3)) []\",\"dB(St(3,3)) []\"\n";    auto to_dB = [](std::complex<double> val) {
    ...
    {
        f_targets << "\"Freq [GHz]\",\"dB(St(1,1)) []\",\"dB(St(1,2)) []\",\"dB(St(2,2)) []\",\"dB(St(1,3)) []\",\"dB(St(2,3)) []\",\"dB(St(3,3)) []\"\n";        for (size_t i = 0; i < freqs.size(); ++i)
        {
            double f = freqs[i];
            auto S = wpd.system_S_matrix(Zref, out2, out3, f, 50);
            f_targets << (f / 1e9) << ","
...
{
    if (!ntl_profiles(tj.get_ntl2(), folder, file + "_ntl2"))   return false;
    if (!ntl_profiles(tj.get_ntl3(), folder, file + "_ntl3"))   return false;


    return true;
}

bool tj_sparams(const TJ::TJ& tj, double Zref, double f_min, double f_max, double f_step, const std::vector<double>& freqs, const std::string& folder, const std::string& file)
{
    std::filesystem::path dir = std::filesystem::path(folder);
    std::filesystem::create_directory(dir);
    ...
    std::array<std::complex<double>, 3> _Zref;
    _Zref[0] = { Zref, 0 };
    _Zref[1] = { Zref, 0 };
    _Zref[2] = { Zref, 0 };
...
bool tj_table_data(const TJ::TJ& tj, const std::string& folder, const std::string& file)
{
    ...
    std::ofstream f_coef(dir / (file + "_coeffs.csv"));
    if (f_coef.is_open()) {
        f_coef << "Term,Arm2,Arm3\n";

        auto ntl2 = tj.get_ntl2();
        auto ntl3 = tj.get_ntl3();
