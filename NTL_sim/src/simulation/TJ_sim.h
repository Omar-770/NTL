...
namespace TJ
{
	...
	public:
		...
		QMainWindow* merge(const char* title = "TJ_sim");

		std::vector<QMainWindow*> z_profile(const TJ& tj);
		std::vector<QMainWindow*> w_h_profile(const TJ& tj);

		std::vector<QMainWindow*> sparams(const TJ& tj, const std::array<std::complex<double>, 3>& Zl, int K = 50);
