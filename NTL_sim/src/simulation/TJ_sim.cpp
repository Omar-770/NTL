...
namespace TJ
{
	...
	std::vector<QMainWindow*> sim::z_profile(const TJ& tj)
	{
		QMainWindow* window1 = NTL_sim.z_profile(tj.get_ntl2(), "NTL2");
		QMainWindow* window2 = NTL_sim.z_profile(tj.get_ntl3(), "NTL3");
	...
	std::vector<QMainWindow*> sim::w_h_profile(const TJ& tj)
	{
		QMainWindow* window1 = NTL_sim.w_h_profile(tj.get_ntl2(), "NTL2");
		QMainWindow* window2 = NTL_sim.w_h_profile(tj.get_ntl3(), "NTL3");
		m_windows.push_back(window1);
		m_windows.push_back(window2);
	...
	std::vector<QMainWindow*> sim::sparams(const TJ& tj, const std::array<std::complex<double>, 3>& Zl, int K)
	{
		for (auto& z : Zl)
			if (std::abs(z) < 1e-12)
		...
		{
			matrix3x3cd S_matrix = tj.S_matrix(f, Zl, K);
			...
			if (log_freq != log_freq_end && f >= *log_freq - 1e-6)
			{
				...
				std::cout << "Matching:\n";
				std::cout << "\tS11: " << 20 * std::log10(std::max(std::abs(S_matrix(0, 0)), 1e-15)) << '\n';
				std::cout << "\tS22: " << 20 * std::log10(std::max(std::abs(S_matrix(1, 1)), 1e-15)) << '\n';
				std::cout << "\tS33: " << 20 * std::log10(std::max(std::abs(S_matrix(2, 2)), 1e-15)) << '\n';
				std::cout << "Split:\n";
				std::cout << "\tS12: " << 20 * std::log10(std::max(std::abs(S_matrix(0, 1)), 1e-15)) << '\n';
				std::cout << "\tS13: " << 20 * std::log10(std::max(std::abs(S_matrix(0, 2)), 1e-15)) << '\n';
				std::cout << "Isolation:\n";
				std::cout << "\tS23: " << 20 * std::log10(std::max(std::abs(S_matrix(1, 2)), 1e-15)) << "\n\n";
				log_freq++;
			}
		}
