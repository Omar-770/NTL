...
namespace WPD
{
	...
	opt_result opt::optimise(console mode)
	{
		...
		return opt_result{ {}, m_wpd, m_ntl[2], m_ntl[3], m_out2_setup, m_out3_setup };
	}
	...
	double opt::min_objective(const std::vector<double>& Cn) const
	{
		...
		double _dz = m_d / m_K;
		std::vector<double> Z2((m_M == 0) ? (m_K % 2 == 0 ? m_K / 2 : m_K / 2 + 1) : m_K);
		std::vector<double> Z3(Z2.size());
		std::vector<double> e_eff2(Z2.size());
		std::vector<double> e_eff3(Z2.size());

		for (int i = 0; i < Z2.size(); i++)
		{
			Z2[i] = NTL::calculate_Z(m_Z0, m_d, Cn2, m_M, (double(i) + 0.5) * _dz);
			Z3[i] = NTL::calculate_Z(m_Z0, m_d, Cn3, m_M, (double(i) + 0.5) * _dz);
			e_eff2[i] = NTL::calculate_er_eff(Z2[i], m_er);
			e_eff3[i] = NTL::calculate_er_eff(Z3[i], m_er);
		}
...
#pragma omp for nowait
			for (int i = 0; i < m_F; i++)
			{
				...
				matrix2x2cd T2 = NTL::calculate_T_matrix(Z2, e_eff2, m_d, m_M, f, m_K);
				...
				matrix2x2cd T3 = NTL::calculate_T_matrix(Z3, e_eff3, m_d, m_M, f, m_K);
				...
				thread_sum_squares += 1.4 * std::pow(std::abs(gamma2), 2);
				thread_sum_squares += 1.4 * std::pow(std::abs(gamma3), 2);

				//A/B   

				thread_sum_squares += 1 * std::pow(std::abs(A3.real() - A2.real()), 2);

				double b2_imag = B2.imag();
				double b3_imag = B3.imag();

				if (std::abs(b2_imag) < 1e-12) b2_imag = (b2_imag < 0.0) ? -1e-12 : 1e-12;
				if (std::abs(b3_imag) < 1e-12) b3_imag = (b3_imag < 0.0) ? -1e-12 : 1e-12;

				thread_sum_squares += 1 * std::pow(std::abs(A2.real() / b2_imag), 2);
				thread_sum_squares += 1 * std::pow(std::abs(A3.real() / b3_imag), 2);
			}
